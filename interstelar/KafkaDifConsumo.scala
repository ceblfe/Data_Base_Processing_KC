...
object KafkaDifConsumo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //val historicoBatch = sc.textFile("out/id_nave_nombre_consumo_medio.csv/part-00000.csv")
    ...
    // Window: 300 sg
    // Slide window: 1 sg
    val resultsDifConsumosStream = difConsumoHistRT.reduceByKeyAndWindow(_ + _,_ - _,Seconds(300),Seconds(1))

    //Imprimimos los resultsDifConsumosStream
    resultsDifConsumosStream.print()



    resultsDifConsumosStream.saveAsTextFiles("out/dif_consumos_medios_hist_rt.csv")
